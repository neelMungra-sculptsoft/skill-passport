...
export default function CertificateModal({ certificate, onClose }: CertificateModalProps) {
  return (
    <div
      ...
      onClick={onClose}
    >
      <div className="max-w-135 w-full" onClick={(e) => e.stopPropagation()}>
        {/* Certificate document */}
        <div className="bg-white rounded-lg px-9 py-8 relative overflow-hidden min-h-85">
          {/* Decorative background shapes */}
          <div className="absolute w-48 h-48 rounded-full bg-green-400 opacity-15 -bottom-14 -left-10 pointer-events-none" />
          <div className="absolute w-40 h-40 rounded-full bg-navy-dark opacity-8 -top-10 right-10 pointer-events-none" />
          <div className="absolute w-32 h-32 rounded-full bg-indigo-500 opacity-12 -top-5 -right-5 pointer-events-none" />

          {/* Header logos */}
          <div className="flex items-center gap-3.5 mb-5">
            <span className="text-2xs font-bold text-gray-700 leading-snug tracking-wide uppercase">
              Singapore
              <br />
              Biodesign
            </span>
            ...
            <div className="flex flex-col">
              <span className="text-2xl font-bold text-green-500 leading-none tracking-tight">
                Lumi
              </span>
              <span className="text-3xs text-gray-400 tracking-widest">Igniting Futures</span>
            </div>
          </div>

          {/* Body */}
          <div className="text-center relative z-10">
            <h2 className="text-2xl font-bold text-gray-900 mb-2.5">Certificate of Distinction</h2>
            <p className="text-sm text-gray-500 mb-2">This certificate is presented to</p>
            <p className="text-lg font-bold text-gray-900 underline mb-2.5">
            ...
            <div className="flex justify-center gap-12 mb-4">
              {[
                ...
                <div key={sig.name} className="text-center">
                  ...
                  <p className="text-xs font-semibold text-gray-900">{sig.name}</p>
                  {sig.role.map((r) => (
                    <p key={r} className="text-2xs text-gray-500">
                      {r}
                    </p>
                  ))}
                </div>
            ...
            <p className="text-2xs text-gray-400 text-right">Certification ID: JWRDKB</p>
          </div>
        </div>
